...
- `pipeline` — `DevelopParams` (the user-facing knob values + `fingerprint()` for cache invalidation), `develop_preview` (libraw `half_size=true` for fast culling), `develop_full` (full-resolution export). RAW path: `read_camera_linear` → resize → Temperature/Tint → CameraToWorking → Exposure → Look + LookStrength (in-linear blend) → tone batch → Rec2020ToSrgb → LinearToOklab → Warmth → OklabToOklch → SoftHighlightsChroma + Color + Clarity → OklchToOklab → OklabToLinear → Grain → SrgbEncode. Image-format path: `read_image_pixels` (decode + resize) → `SrgbDecode` → Temperature/Tint as per-channel gain (the buffer is re-tagged from `LinearSrgb` to `CameraLinear` so the existing controls apply) → `SrgbToRec2020` → identical knob chain to the RAW path from Exposure onward. The only knobs that don't apply to image-format input are the sensor-specific ones gated upstream; everything else maps cleanly.
...
- `app` — framework-agnostic state and update logic. Owns the `Db`, the `FileEntry` list (each carries persisted `develop_params`, `develop_params_fp`, `source_fp`, and `removed`), the `visible` index list (after filter + show-removed), the `enabled_formats` set, the `develop_params: DevelopParams` live editing buffer, the `develop_cursor: usize` over the `DEVELOP_KNOBS` table, the `view: View` (`Main`/`Filter`), the `focus: Focus` (`Navigation`/`Develop`), the `show_removed: bool`, and a `file_meta: HashMap<PathBuf, FileMeta>` cache populated lazily by the preview worker. Methods (`next`, `prev`, `remove_current`, `restore_current`, `toggle_show_removed`, `commit_develop_params`, `sync_develop_params_from_current`, `toggle_format`, `open_filter`/`close_filter`, `filter_next`/`filter_prev`, `toggle_current_filter`, `enter_develop`/`exit_develop`, `develop_next`/`develop_prev`, `develop_adjust`, `develop_reset`) are pure state mutations — DB writes are limited to `set_removed` (synchronous in `remove_current`/`restore_current`) and the debounced `update_params` flush triggered from the TUI loop.

The TUI half lives under `crates/terminalroom/src/tui/`:

- `tui::run` — terminal setup with a Drop guard, `Picker::from_query_stdio()` (with halfblocks fallback). Spawns one worker thread that calls `codec::decode` + `pipeline::develop_preview`, plus one event-reader thread that pumps crossterm events through a channel. Main loop runs `crossbeam_channel::select!` over events, completed jobs, and a debounce-aware timeout (≤ 100 ms tick, but bounded by the remaining 250 ms knob debounce). Two cache tiers: a hot `LruCache<PathBuf, PreviewEntry>` (cap 9) where `PreviewEntry` holds `proto: StatefulProtocol`, the rendered source dimensions, the `TargetSize` it was rendered for, and the `params_fingerprint` of the `DevelopParams` that produced it; and the warm on-disk `Cache` from `crate::cache`. Selection change or ≥ 25% target change tries the disk cache before enqueueing a worker job. Knob edits run a debounce timer: `pending_develop_change(app)` (live params fingerprint differs from the entry's committed fingerprint) starts the 250 ms timer, and on expiry `flush_pending_develop` writes the JSON + fingerprint to the DB and commits onto the entry — that fingerprint change then drives the render decision in the next iteration. Force-flush points: navigation (`j`/`k`), `x`/`r`/`R`, `f`, `Enter`, `Esc`, and the `Quit` exit branch. Each completed `JobDone` also carries an `Option<FileMeta>` (shot info + dims + size + kind) that `handle_job_done` writes into `app.file_meta` and persists the `Srgb8` to the on-disk cache before installing the in-memory `PreviewEntry`.
...
8. Start the TUI; spawn the worker thread and the event-reader thread; pass the on-disk `Cache` in.
9. On each loop iteration:
   - Detect any pending develop edit (live `app.develop_params.fingerprint()` differs from the current entry's `develop_params_fp`). If pending and the 250 ms debounce has elapsed, persist via `Db::update_params` and `app.commit_develop_params(fp)`.
   ...
   - On selection change — or a target change ≥ 25% in either dim — or a committed params fingerprint change — try the in-memory `LruCache` first; on miss, try the on-disk `Cache::get` (`source_fp` + committed `params_fp`). On disk-cache hit, install in the in-memory tier. On miss, bump `current_generation`, flip the prior selection's `Arc<AtomicBool>` cancel flag, and enqueue a job carrying a clone of `DevelopParams`, `params_fingerprint`, `source_fp`, and the file's `size_bytes`.
   - Bound the `crossbeam_channel::select!` timeout by the remaining 250 ms debounce so the flush fires close to schedule.
10. The worker calls `codec::decode(path)` to get a `Loaded`, builds a `FileMeta` from it (shot info, dims, orientation, kind, plus the `size_bytes` from the job), then runs `pipeline::develop_preview(&loaded, &params, target, cancel)` to produce an `Srgb8`. For RAW this calls `read_camera_linear` (libraw `half_size=true`, `output_color=Raw`, `use_camera_wb=false`), wraps in `Buffer<CameraLinear>`, resizes to target, applies `CameraToWorking` (WB + cam→Rec.2020 matrix from `SensorInfo`), runs the 12-knob chain, and encodes via `SrgbEncode`. For image files it lazy-decodes via `read_image_pixels` (JPEG IDCT scale; PNG/TIFF full decode) and resizes. The `Srgb8`, `FileMeta`, and `source_fp` come back together through a channel; stale generations are dropped on receive. On the UI thread the result is first persisted to the on-disk cache via `Cache::insert` (atomic write + `set_cache_key` + `touch_access` + LRU eviction), then wrapped in `DynamicImage::ImageRgb8`, handed to `picker.new_resize_protocol`, and stored in the in-memory cache alongside the source dims, the target, and the params fingerprint that produced it; the `FileMeta` is written into `app.file_meta` so the Image Info tab can render shoot info.
...
13. Shift focus to the Develop tab when `Enter` is pressed in Navigation focus; `j/k` then move between knobs, `h/l` adjust the focused knob by its step, `r` resets the focused knob to its default, `Esc` returns focus to Navigation. Knob edits accumulate in `app.develop_params` and are flushed to the DB after the 250 ms debounce; the same debounce gates re-rendering. Image-navigation keys (`x`/`r`/`R`/`f`) are inert in Develop focus.
...
- `db: Db` — owned SQLite handle for state writes (the global DB).
- `develop_params: DevelopParams` — live editing buffer for the currently-selected file's knobs. On selection change `sync_develop_params_from_current` copies the new entry's persisted params here. After the 250 ms debounce, `commit_develop_params(fp)` writes the new value back onto the entry.
...
The TUI layer (`tui::run`) owns the concurrency-and-rendering state outside `App`: the `Picker`, the on-disk `Cache`, the `LruCache<PathBuf, PreviewEntry>`, the `crossbeam_channel` senders/receivers, the `current_cancel: Option<Arc<AtomicBool>>`, the per-path `latest_generation: HashMap<PathBuf, u64>`, the `last_id: Option<i64>` / `last_target` / `last_rendered_fp` change-detection state, the `params_dirty_at: Option<Instant>` debounce timer, and the worker/event threads. These are intentionally outside `App` so the headless logic stays free of ratatui and threading types.
...
Two cache tiers back the same render path:

- **Hot tier**: `LruCache<PathBuf, PreviewEntry>` (cap 9) keyed by canonical path, holding the `StatefulProtocol` ratatui-image needs to re-paint without re-decoding.
- **Warm tier**: on-disk `Cache` at `~/.terminalroom/cache/` keyed by BLAKE3-of-canonical-path, holding the `Srgb8` pixel buffer (header + raw RGB bytes). The `cache_key` reference and `last_access_unix_seconds` LRU clock live in the global DB.

On selection change or target change, the loop tries the hot tier; on miss, the warm tier; on miss again, the worker is asked to develop. On knob change, the same flow runs but is gated by the 250 ms debounce — so quickly held arrow keys settle into one re-render after release. Selecting a file whose persisted develop has been seen before pulls the developed `Srgb8` straight from disk and skips the worker entirely.

The worker is unchanged in shape:

- **Default render** (Navigation focus): `pipeline::develop_preview(loaded, &app.develop_params, target, cancel)` — libraw `half_size=true` (~4× faster demosaic); develop at TUI target resolution. New files default to identity `DevelopParams`; existing files use their persisted knobs.
- **Develop focus**: same call site, but the user has been adjusting `app.develop_params`; after the 250 ms debounce the new params are committed and a re-render is triggered.
...
- If a debounced develop flush fails to write, surface the error in `app.status` and keep the in-memory edits — the next debounce tick retries.
