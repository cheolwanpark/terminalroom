...
- TUI preview loading runs on a dedicated develop worker thread fed by two `crossbeam-channel`s: a `bounded(1)` foreground channel for the active selection (with `try_send` semantics — the cancel-flag pattern supersedes stale jobs) and a `bounded(2)` prefetch channel for cursor±1 warm-up. The worker drains foreground first; prefetch only runs when the user is idle. The pipeline is split into `prepare_source` (decode + resize, param-independent) and `apply_pipeline` (param-dependent), and the worker keeps an `LruCache<SourceKey, Arc<PreparedSource>>` (cap 3) keyed by `(path, source_fp, target_bucket)` so knob ticks on the active file skip libraw demosaic / JPEG IDCT entirely. Three preview cache tiers back the same render path: a worker-side prepared-source cache (above), an in-memory `LruCache<PathBuf, PreviewEntry>` (cap 15, hot tier) keyed by canonical path with a `params_fingerprint` per entry, and a persistent on-disk cache at `~/.terminalroom/cache/` (LRU-bounded to 500 entries) keyed by BLAKE3-of-canonical-path. A separate save worker thread owns its own `Db` connection and a clone of the disk cache; `flush_pending_develop` and the on-disk-cache write run there, so the UI thread never blocks on `fs::sync_all` or rusqlite during a knob tick. WAL + `PRAGMA busy_timeout = 5000` lets the save worker and the main thread serialize DB writes cleanly. Knob debounce is tiered (50 ms when the source is in the worker's cache, 250 ms otherwise). Held-key navigation goes through a coalescer that rate-limits cursor advances to 5 Hz for the first second of the burst and full OS-repeat speed thereafter; image dispatch and disk-cache installs are both gated on the user releasing the key (`NAV_SETTLE = 150 ms` of nav-input silence, anchored on the most recent nav event so a long ramp-up doesn't accidentally settle between rate-limited advances). The preview pane is frozen on the last settled path during a burst via a `displayed_path` separate from the live cursor — so the pane doesn't flicker through every previously-cached file the cursor scrolls over.
- Kitty graphics-storage hygiene: each `picker.new_resize_protocol(...)` mints a 32-bit kitty image id and transmits the image bytes once per protocol lifetime; subsequent renders are unicode-placeholder cells that reference the id. When the in-memory `LruCache<PathBuf, PreviewEntry>` evicts (or replaces) an entry, the TUI emits a kitty `_Ga=d,d=I,i={id};` delete escape so the terminal frees the image's quota. Without this, kitty/Ghostty/WezTerm exceed their default 320 MB quota after a few hundred previews and start evicting images that placeholders still reference, which would surface as a fully black preview pane. No-op on sixel/iterm2/halfblocks backends.
...
- Develop knobs are persisted per file. Knob changes commit in-memory immediately (so the next render-decision sees the new fingerprint) and are persisted asynchronously by the save worker. Debounce is tiered: 50 ms when the worker has the active source cached (the common case after the first preview lands), 250 ms otherwise. Force-flush on selection change, focus change, and quit queues the write through the save worker; on quit, the worker is joined before the alternate-screen guard tears down so pending writes always land.
...
- [x] Single-screen TUI (ratatui + ratatui-image, three-tier preview cache with `params_fingerprint`, format filter popup, aspect-fit centered preview, ASCII banner with single-row fallback).
- [x] Develop tab + focus model (`Enter` enters, `Esc` exits; in-focus knob list driven by `j/k/h/l/r`; tiered-debounce re-render + async DB commit on knob change).
- [x] Per-file develop persistence (knob edits debounced 50/250 ms; force-flush on selection/focus/quit routes through the save worker, joined on shutdown).
- [x] Hot-path performance: pipeline split into `prepare_source` + `apply_pipeline`, worker-side `Arc<PreparedSource>` cache (cap 3), event coalescing, prefetch ±1 with priority queue, navigation-settle gate.
