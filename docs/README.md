...
- **The develop pipeline owns the camera→working transform end-to-end.** `read_camera_linear` calls libraw with `output_color=Raw` and `use_camera_wb=false`; darkroom's `CameraToWorking` then applies WB multipliers and the camera→Rec.2020 matrix (composed offline from libraw's `cam_to_xyz` and the constant XYZ→Rec.2020 D65 matrix). This lets Temperature/Tint operate on camera-native data before WB.
...
- [x] `codec::decode_raw` — `Raw` header (shot-info + sensor-info from libraw); `read_camera_linear` returns planar f32 camera-linear (libraw `output_color=Raw`, `use_camera_wb=false`).
